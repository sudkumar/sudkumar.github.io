...
Now we further request for next 2 people, we can either get [P4, P3] or [P1, P3]. Hence, we can get P1 in the first result as well as in the second result set! **This is 
one of the reason we get duplicate items on pagination**.


As you can see, **the question of stable and non-stable sorting comes only when we have items with the same criteria/comparision value**. Can we
avoid it ?
...
Similar to this example, we can implement a strategy for **unique sorting** and ensure a stable order of items if they have the same field value. 
